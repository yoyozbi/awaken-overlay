...
export default class RegisterCommand implements Command {
  ...
  async run(interaction: CommandInteraction): Promise<void> {
    ...
    await this.calendarService
      .createCalendar({ guildId: guild.id, googleCalendarId: calendarId?.toString(), parseInt(numberOfDays), interval, channelId: channel.toString() })


    await interaction.reply({ content: "I will now watch your calendar", ephemeral: true });
  }
}
